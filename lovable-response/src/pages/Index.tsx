...
import { ArrowRight, BookOpen, Search, Zap, Server, ShieldCheck, GitBranch } from "lucide-react";
...
const Index = () => {
  ...
  const navQuery = useQuery({
    queryKey: ["wiki-nav", primaryGroup?.id],
    queryFn: () => fetchWikiNav(primaryGroup!.id),
    enabled: Boolean(primaryGroup?.id),
    initialData: primaryGroup
      ? { success: true, data: { groupId: primaryGroup.id, nodes: wikiNavTree[primaryGroup.id] ?? [] }, error: null }
      : undefined,
  });

  const quickLinks = useMemo(() => {
    const nodes = navQuery.data?.data?.nodes ?? [];
    return [
      nodes.find((node) => node.fullPath === "kakao/oauth/intro"),
      nodes.find((node) => node.fullPath === "api-reference/wiki-nav"),
      nodes.find((node) => node.fullPath === "kakao/oauth/console"),
            ...
            <div className="max-w-2xl animate-fade-in space-y-4">
              ...
              <div>
                ...
                <p className="text-lg text-muted-foreground leading-relaxed">
                  REST API 연동부터 OAuth 인증, 플러그인 개발까지. {tenantInfo.tagline}
                  핵심 API 스키마를 그대로 써서 테넌트/그룹/문서 구조를 검증할 수 있습니다.
                </p>
              </div>
              <div className="flex flex-wrap gap-3">
                <Button
                  ...
                  onClick={() =>
                    defaultPath && navigate(`/docs/${defaultPath}?groupId=${primaryGroup?.id ?? "api-guide"}`)
                  }
                >
                ...
                <Button size="lg" variant="ghost" onClick={() => navigate("/auth/login")}>
                  로그인/회원가입
                </Button>
              </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {quickLinks.length > 0 ? (
              quickLinks.map((item) => (
                <button
                  key={item.id}
                  onClick={() => navigate(`/docs/${item.fullPath}?groupId=${primaryGroup?.id ?? "api-guide"}`)}
        ...
        <section className="container max-w-5xl py-6 px-6">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            {groups.map((group) => (
              <div
                key={group.id}
                className="p-4 rounded-xl border border-border bg-card shadow-theme-sm flex flex-col gap-2"
              >
                ...
                <Button
                  ...
                  onClick={() => navigate(`/docs/${group.defaultPath}?groupId=${group.id}`)}
                >
                  이동하기
                </Button>
              </div>
                ...
                {popularDocs.map((doc) => (
                  <li key={doc.id}>
                    <button
                      ...
                      onClick={() => navigate(`/docs/${doc.path}?groupId=${doc.groupId}`)}
                    >
                      {doc.title}
                    </button>
                  </li>
