...
import { popularDocs, recentDocs, docGroups } from '@/lib/mockData';
import { ArrowRight, BookOpen, Search, Zap, Code } from 'lucide-react';
          ...
          <div className="relative container max-w-5xl py-16 px-6">
            <div className="max-w-2xl animate-fade-in">
              ...
              <div className="flex flex-wrap gap-3">
                <Button size="lg" className="shadow-lg hover-glow" onClick={() => navigate('/docs/getting-started')}>
                  시작하기 <ArrowRight className="ml-2 h-4 w-4" />
                </Button>
                <Button size="lg" variant="outline" onClick={() => navigate('/docs/getting-started')}>
                  문서 바로가기
                </Button>
        ...
        <section className="container max-w-5xl py-12 px-6">
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              { icon: Code, title: 'API 레퍼런스', desc: '모든 엔드포인트와 파라미터를 상세히 문서화했습니다.', link: '/docs/api-reference' },
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="p-5 rounded-xl border border-border bg-card shadow-theme-sm">
              ...
              <ul className="space-y-2">
                {popularDocs.map((doc) => (
                  <li key={doc.id}>
                    <Link to="/docs/getting-started" className="text-sm text-muted-foreground hover:text-primary transition-colors">
                      {doc.title}
                    </Link>
                  </li>
                ))}
